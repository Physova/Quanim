...
const SUN_FRAG = `
  ...
  void main() {
    ...
    vec3 hotW = vec3(1.0, 0.98, 0.88);
    vec3 warm = vec3(1.0, 0.62, 0.08);
    vec3 orng = vec3(0.95, 0.40, 0.02);
    vec3 cool = vec3(0.72, 0.20, 0.02);
    ...
    col = mix(col, umbra, 0.7);
    col = mix(col, penumbra, spots * 0.35);
    col *= 0.60 + limb * 0.54;
...
const CHROMO_FRAG = `
  ...
  varying vec3 vNorm;
  void main() {
    float f = 1.0 - abs(dot(vNorm, vec3(0, 0, 1)));
    float ch = pow(f, 1.6);
    vec3 col = mix(vec3(1.0, 0.55, 0.06), vec3(1.0, 0.18, 0.01), ch);
    gl_FragColor = vec4(col, ch * 0.52 * uOpa);
  }
`;
...
function mkEarthTex(): THREE.CanvasTexture {
  ...
  og.addColorStop(0, 'rgba(10,90,180,.3)');
  ...
  x.fillStyle = '#145520';
  ([
    [165, 155, 70, 60], [200, 235, 38, 60], [445, 142, 44, 38], [600, 132, 120, 70]
  ] as number[][]).forEach(([cx, cy, rw, rh]) => {
    x.beginPath(); x.ellipse(cx, cy, rw / 2, rh / 2, 0, 0, Math.PI * 2); x.fill();
  });
  ...
  ([
    [160, 150], [195, 230], [440, 138], [595, 128], [698, 208], [742, 328]
  ] as number[][]).forEach(([cx, cy]) => {
    for (let i = 0; i < 22; i++) {
      x.beginPath();
      x.arc(cx + (Math.random() - 0.5) * 72, cy + (Math.random() - 0.5) * 50, Math.random() * 1.4 + 0.5, 0, Math.PI * 2);
      x.fill();
    }
  });
  x.globalAlpha = 1;
...
function mkWingGeo(span: number, chord: number, taper: number): THREE.BufferGeometry {
  ...
  const v = new Float32Array([
    chord * 0.3, 0.012, 0, chord * 0.3, 0.012, hw,
    -chord * 0.7, 0.012, 0, -chord * taper, 0.012, hw,
    chord * 0.3, -0.012, 0, chord * 0.3, -0.012, hw,
    -chord * 0.7, -0.012, 0, -chord * taper, -0.012, hw,
  ]);
...
export default function QuanimHero() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < SN; i++) {
      sb[i * 3] = (Math.random() - 0.5) * 220;
      sb[i * 3 + 1] = (Math.random() - 0.5) * 220;
      sb[i * 3 + 2] = -30 - Math.random() * 80;
    }
    ...
    const photonRM = new THREE.MeshBasicMaterial({
      color: 0xfff0cc, transparent: true, opacity: 0, side: THREE.DoubleSide,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    const photonR = new THREE.Mesh(new THREE.RingGeometry(0.975, 1.19, 256), photonRM);
    photonR.rotation.x = Math.PI * 0.27; bhGrp.add(photonR);

    const photonR2M = new THREE.MeshBasicMaterial({
      color: 0x88ccff, transparent: true, opacity: 0, side: THREE.DoubleSide,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    const photonR2 = new THREE.Mesh(new THREE.RingGeometry(0.965, 1.05, 256), photonR2M);
    photonR2.rotation.x = Math.PI * 0.27; bhGrp.add(photonR2);
    ...
    for (let i = 0; i < DN; i++) {
      ...
      const rN = Math.max(0, (r - 1.18) / 2.7);
      const temp = Math.pow(1 - rN, 1.8);
      const ap = (Math.sin(a) + 1) * 0.5;
      dkCol[i * 3] = Math.min(1, temp * 1.1 + 0.15);
      dkCol[i * 3 + 1] = Math.min(1, temp * 0.65 * (0.6 + 0.4 * ap) * 0.8);
      dkCol[i * 3 + 2] = Math.min(1, ap * 0.55 * (1 - temp * 0.6) * 0.35);
    }
    ...
    dkPts.rotation.x = Math.PI * 0.27; bhGrp.add(dkPts);
    ...
    hPts.rotation.x = Math.PI * 0.27; bhGrp.add(hPts);
    ...
    // ── TRANSITION FLARE ──────────────────────────────────────────────────
    const flareSprMat = new THREE.SpriteMaterial({
      map: mkFlareTex(), transparent: true, opacity: 0,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    const flareSpr = new THREE.Sprite(flareSprMat);
    flareSpr.scale.set(0.01, 0.01, 1);
    scene.add(flareSpr);
    ...
    const eMat = new THREE.MeshPhongMaterial({
      map: earthTex, transparent: true, opacity: 0,
      specular: new THREE.Color(0.2, 0.45, 0.9), shininess: 14,
    ...
    // ── TINY SPACECRAFT (near Earth during transition) ────────────────────
    ...
    const tinyBody = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.006, 0.06, 8), tinyShipMat);
    tinyBody.rotation.z = Math.PI / 2; tinyShipGrp.add(tinyBody);
    const tinyNose = new THREE.Mesh(new THREE.ConeGeometry(0.008, 0.02, 8), tinyShipMat);
    tinyNose.rotation.z = -Math.PI / 2; tinyNose.position.x = 0.04; tinyShipGrp.add(tinyNose);
    tinyShipGrp.position.set(1.0, 0.8, 0.5);

    // ── SPACESHIP (Scene 3 — zoomed in) ───────────────────────────────────
    const planeGrp = new THREE.Group(); scene.add(planeGrp);
    planeGrp.scale.setScalar(1.8);

    const mkSolid = (col: number, spec = 0x6688aa, shin = 70) => new THREE.MeshPhongMaterial({
      color: col, specular: spec, shininess: shin, transparent: true, opacity: 0,
      depthWrite: true, side: THREE.FrontSide,
    });
    const mBod = mkSolid(0xdddddd, 0xffffff, 50);
    const mMet = mkSolid(0xaaaaaa, 0x8899aa, 90);
    const mDrk = mkSolid(0x222222, 0x111111, 20);
    const mGls = mkSolid(0x08121e, 0xffffff, 210);
    ...
    for (let i = 0; i <= 24; i++) {
      const t = i / 24;
      let r: number;
      if (t < 0.15) { r = 0.01 + Math.sin(t / 0.15 * Math.PI * 0.5) * 0.11; }
      else if (t < 0.85) { r = 0.12; }
      else { r = 0.12 * (1 - (t - 0.85) / 0.15 * 0.3); }
      fProf.push(new THREE.Vector2(r, t * 3.0 - 1.5));
    }
    addM(new THREE.LatheGeometry(fProf, 24), mBod, 0, 0, 0, 0, 0, Math.PI / 2);
    addM(new THREE.SphereGeometry(0.015, 12, 12), mMet, 1.55, 0, 0);

    // Wings
    const wR = new THREE.Mesh(mkWingGeo(0.7, 1.4, 0.15), mMet);
    wR.position.set(-0.2, 0, 0); planeGrp.add(wR);
    const wLg = mkWingGeo(0.7, 1.4, 0.15);
    wLg.scale(-1, 1, -1); wLg.computeVertexNormals();
    const wL = new THREE.Mesh(wLg, mMet);
    wL.position.set(-0.2, 0, 0); planeGrp.add(wL);
    ...
    const mkThruster = (px: number, py: number, pz: number) => {
      ...
      const tb = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.045, 0.28, 14), mMet);
      tb.rotation.set(0, 0, Math.PI / 2); g.add(tb);
      const er = new THREE.Mesh(new THREE.TorusGeometry(0.05, 0.007, 8, 20), mDrk);
      er.rotation.y = Math.PI / 2; er.position.x = -0.14; g.add(er);
      planeGrp.add(g);
    };
    mkThruster(-1.5, 0, 0);
    mkThruster(-1.45, -0.07, 0.10);
    mkThruster(-1.45, -0.07, -0.10);

    // Collect all plane materials for opacity control
    const planeMats: THREE.Material[] = [];
    planeGrp.traverse((c: THREE.Object3D) => {
      if ((c as THREE.Mesh).isMesh && (c as THREE.Mesh).material) {
        planeMats.push((c as THREE.Mesh).material as THREE.Material);
      }
    });

    // Exhaust particles
    const HEAT_N = 450;
    const hxBuf = new Float32Array(HEAT_N * 3);
    const hxLife = new Float32Array(HEAT_N);
    const hxCol = new Float32Array(HEAT_N * 3);
    for (let i = 0; i < HEAT_N; i++) {
      hxLife[i] = Math.random();
      const thIdx = i % 3;
      let ty = 0, tz = 0;
      if (thIdx === 1) { ty = -0.07; tz = 0.10; } else if (thIdx === 2) { ty = -0.07; tz = -0.10; }
      hxBuf[i * 3] = -1.5 + (Math.random() - 0.5) * 0.04;
      hxBuf[i * 3 + 1] = ty;
      hxBuf[i * 3 + 2] = tz + (Math.random() - 0.5) * 0.04;
      const t2 = Math.random();
      hxCol[i * 3] = 1; hxCol[i * 3 + 1] = t2 * 0.4; hxCol[i * 3 + 2] = 0;
    }
    const hxG = new THREE.BufferGeometry();
    hxG.setAttribute('position', new THREE.BufferAttribute(hxBuf, 3));
    hxG.setAttribute('color', new THREE.BufferAttribute(hxCol, 3));
    const hxM = new THREE.PointsMaterial({
    ...
    // Corona — 7 layers
    const coronaDefs = [
      { r: 2.5, col: 0xff8800, max: 0.18 }, { r: 3.3, col: 0xff5500, max: 0.11 },
      { r: 4.4, col: 0xff2200, max: 0.055 }, { r: 5.8, col: 0xff1100, max: 0.035 },
      { r: 7.5, col: 0xdd0800, max: 0.02 }, { r: 10, col: 0xcc0600, max: 0.012 },
      { r: 13, col: 0xaa0400, max: 0.006 },
    ];
    ...
    const promPts = [
      [2, 0, 0.1], [3.1, 1.5, 0.2], [3.2, 2.5, -0.1], [1.9, 1.2, 0.2],
      [-1.8, 0.9, 0.3], [-3, 2.2, 0.5], [-3.2, 3, 0.2], [-2.3, 1, -0.2],
      [0.5, 2, 0.1], [1.3, 3.2, 0.4], [-0.5, 3.5, 0.5], [-1, 2, 0.1],
      [1.6, -0.5, 0.5], [2.5, -1.8, 0.3], [2.8, -2.8, -0.1], [1.4, -1, 0.4],
    ];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const m = new THREE.LineBasicMaterial({
        color: promColors[i], transparent: true, opacity: 0,
        blending: THREE.AdditiveBlending, depthWrite: false,
      });
    ...
    // Solar flares — elongated spheres
    const sunFlareDefs = [
      { dir: [0.8, 0.5, 0], sc: [0.08, 0.55, 0.08] as [number, number, number], col: 0xff7700 },
      { dir: [-0.4, 0.9, 0.3], sc: [0.07, 0.42, 0.07] as [number, number, number], col: 0xff4400 },
      { dir: [0.2, -0.85, 0.5], sc: [0.06, 0.38, 0.06] as [number, number, number], col: 0xff8800 },
      { dir: [0.9, -0.3, -0.2], sc: [0.06, 0.32, 0.06] as [number, number, number], col: 0xff5500 },
    ];
    const sunFlareMats = sunFlareDefs.map(({ dir, sc, col }) => {
      ...
      mesh.position.set(dir[0] / L * 2.4, dir[1] / L * 2.4, dir[2] / L * 2.4);
      sGrp.add(mesh);
      return m;
    });
    ...
    // Orbiting planets
    const planetDefs = [
      { radius: 3.8, size: 0.06, color: 0x888888, speed: 1.6, incl: 0.03, isEarth: false },
      { radius: 5.2, size: 0.09, color: 0xddaa55, speed: 1.1, incl: -0.05, isEarth: false },
      { radius: 7.0, size: 0.10, color: 0x2266aa, speed: 0.7, incl: 0.04, isEarth: true },
      { radius: 9.0, size: 0.08, color: 0xcc5533, speed: 0.5, incl: 0.08, isEarth: false },
    ];
    const planets = planetDefs.map(({ radius, size, color, speed, incl, isEarth }) => {
      ...
      const mesh = new THREE.Mesh(
        new THREE.SphereGeometry(size, isEarth ? 32 : 12, isEarth ? 32 : 12), mat
      );
      sGrp.add(mesh);
      ...
      for (let a = 0; a <= 64; a++) {
        ...
        oPts.push(new THREE.Vector3(Math.cos(ang) * radius, 0, Math.sin(ang) * radius));
      }
      const oLine = new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints(oPts), oMat);
      oLine.rotation.x = Math.PI * 0.42 + incl;
      sGrp.add(oLine);
    ...
    flashEl.className = 'qh-flash';
    document.body.appendChild(flashEl);

    // ── DOM: Label System ─────────────────────────────────────────────────
    // Container for labels
    const labelRoot = document.createElement('div');
    labelRoot.className = 'qh-label-root';
    document.body.appendChild(labelRoot);

    // SVG for leader lines
    const svgEl = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svgEl.setAttribute('class', 'qh-label-svg');
    svgEl.setAttribute('width', '100%');
    svgEl.setAttribute('height', '100%');
    document.body.appendChild(svgEl);

    // SVG lines
    const lnHeat = mkSvgLine(svgEl);
    const lnFuel = mkSvgLine(svgEl);
    const lnAero = mkSvgLine(svgEl);
    const lnCore = mkSvgLine(svgEl);
    const lnSurf = mkSvgLine(svgEl);
    const lnLum = mkSvgLine(svgEl);
    ...
    function proj3D(v3: THREE.Vector3): { x: number; y: number } {
      const v = v3.clone().project(camera);
      return { x: (v.x * 0.5 + 0.5) * window.innerWidth, y: (-v.y * 0.5 + 0.5) * window.innerHeight };
    }

    function setLabel(
      el: HTMLDivElement, ln: SVGLineElement,
    ...
    function animate(now: number) {
      ...
      const dt = Math.min(0.05, (now - prevT) * 0.001);
      prevT = now;
      ...
      // ── Black Hole ──
      const bhA = 1.0 * (1 - ss(0.20, 0.30, s));
      bhSphereMat.opacity = bhA;
      bhGrp.visible = bhA > 0.001;
      ...
      if (flareA > 0.005) {
        ...
        flareSpr.scale.set(bloom * 5, bloom * 5, 1);
      } else { flareSpr.scale.set(0.01, 0.01, 1); }
      flashEl.style.opacity = String(flashV * 0.22);

      // ── Earth (stays centered) ──
      const eA = ss(0.30, 0.38, s) * (1 - ss(0.42, 0.48, s));
      ...
      if (eGrp.visible) {
        ...
        const zoom = 1 - ss(0.30, 0.48, s);
        eGrp.scale.setScalar(0.8 + zoom * 1.4);
        eGrp.position.set(0, 0, 0);
      }

      // ── Tiny spacecraft near Earth ──
      const tinyA = ss(0.38, 0.42, s) * (1 - ss(0.45, 0.48, s));
      tinyShipGrp.visible = tinyA > 0.001;
      tinyShipMat.opacity = tinyA;

      // ── Spacecraft ──
      const planeA = ss(0.45, 0.55, s) * (1 - ss(0.79, 0.85, s));
      planeGrp.visible = planeA > 0.001;

      if (planeGrp.visible) {
        planeMats.forEach(m => { (m as any).opacity = planeA; });
        planeGrp.rotation.y = Math.sin(now * 0.00012) * 0.04;

        // Move spacecraft UP during bento display (0.45-0.72), back to center after
        const bentoLift = ss(0.45, 0.52, s) * (1 - ss(0.70, 0.74, s));
        planeGrp.position.y = Math.sin(now * 0.00018) * 0.04 + bentoLift * 2.8;
        ...
        if (fx > 0.002) {
          ...
          for (let i = 0; i < HEAT_N; i++) {
            ...
            if (hxLife[i] > 1) {
              ...
              let ty = 0, tz = 0;
              if (thIdx === 1) { ty = -0.07; tz = 0.10; } else if (thIdx === 2) { ty = -0.07; tz = -0.10; }
              ha[i * 3] = -1.5 + (Math.random() - 0.5) * 0.04;
              ha[i * 3 + 1] = ty;
              ha[i * 3 + 2] = tz + (Math.random() - 0.5) * 0.04;
            } else {
              ha[i * 3] -= dt * (0.7 + hxLife[i] * 0.9);
              ha[i * 3 + 1] += (Math.random() - 0.5) * dt * 0.08;
            }
          }
        ...
        if (labA > 0.04) {
          const wh = new THREE.Vector3(1.5, 0, 0);
          const wf = new THREE.Vector3(-1.5, 0, 0);
          const wa = new THREE.Vector3(-0.2, 0, 0.35);
          planeGrp.localToWorld(wh); planeGrp.localToWorld(wf); planeGrp.localToWorld(wa);
          const ph = proj3D(wh), pf = proj3D(wf), paa = proj3D(wa);
          setLabel(lblHeat, lnHeat, ph.x, ph.y, 90, -85, 'rgba(255,155,55,.7)', labA);
          setLabel(lblFuel, lnFuel, pf.x, pf.y, -105, -55, 'rgba(100,235,165,.7)', labA);
          setLabel(lblAero, lnAero, paa.x, paa.y, 80, -105, 'rgba(80,185,255,.7)', labA);
        } else {
          hideLabel(lblHeat, lnHeat); hideLabel(lblFuel, lnFuel); hideLabel(lblAero, lnAero);
        }
      } else {
        hideLabel(lblHeat, lnHeat); hideLabel(lblFuel, lnFuel); hideLabel(lblAero, lnAero);
      }
      ...
      camZ = 5.0 + ultraZoomOut * 7.0;
      camY = ultraZoomOut * 0.2;
      ...
      // ── Sun ──
      // Sun persists at full scroll — no fadeout at bottom
      const sunA = ss(0.86, 0.94, s);
      sGrp.visible = sunA > 0.001;

      if (sGrp.visible) {
        sunMat.uniforms.uOpa.value = sunA;
        chromoMat.uniforms.uOpa.value = sunA;
        sunClock += dt;
        ...
        // Orbiting planets
        planets.forEach((p, i) => {
          p.mat.opacity = sunA * 0.85;
          p.oMat.opacity = sunA * 0.06;
          const angle = now * 0.0003 * p.speed + i * 1.6;
          const r = p.radius;
          p.mesh.position.set(
            Math.cos(angle) * r,
            Math.sin(p.incl) * Math.sin(angle) * r * 0.15,
            Math.sin(angle) * r * 0.4
          );
        });

        // Sun labels
        // Sun labels persists at full scroll — no fadeout
        const sunLabA = ss(0.88, 0.94, s);
        if (sunLabA > 0.04) {
          const W = window.innerWidth, H = window.innerHeight;
          const coreAnc = proj3D(new THREE.Vector3(0, 0, 0));
          setLabel(lblCore, lnCore, coreAnc.x, coreAnc.y, -W * 0.32, H * 0.28, 'rgba(255,220,100,.7)', sunLabA);
          const surfAnc = proj3D(new THREE.Vector3(2.1, 0, 0));
          setLabel(lblSurf, lnSurf, surfAnc.x, surfAnc.y, W * 0.28, -H * 0.25, 'rgba(255,155,55,.7)', sunLabA);
          const lumAnc = proj3D(new THREE.Vector3(-0.5, 2.0, 0));
          setLabel(lblLum, lnLum, lumAnc.x, lumAnc.y, -W * 0.30, -H * 0.22, 'rgba(255,245,185,.7)', sunLabA);
        } else {
          hideLabel(lblCore, lnCore); hideLabel(lblSurf, lnSurf); hideLabel(lblLum, lnLum);
        }
      } else {
        hideLabel(lblCore, lnCore); hideLabel(lblSurf, lnSurf); hideLabel(lblLum, lnLum);
      }
    ...
    return () => {
      ...
      if (labelRoot.parentNode) labelRoot.remove();
      if (svgEl.parentNode) svgEl.remove();
    };
  }, [handleScroll]);
