# Implementation Plan: Physova 3D Narrative 3.0 (High Fidelity)
...
## Key Files
- `components/visuals/physova-hero.tsx` (Logic & Scene)
- `components/visuals/physova-hero.css` (Styles)
- `app/page.tsx` (Integration)

## Phase 1: Singularity & Flare Transition
- [ ] **EXACT Port of 22k Particle System**: Port the 22,000 particle accretion disk, Keplerian velocity rotation loop, and Doppler shifting (dkCol) exactly from `files/PhysovaHero.jsx`. Ensure positions update in the `animate` loop.
...
- [ ] **Port Sun Shader**: FBM turbulence, limb darkening, and 4 corona layers from `files/PhysovaHero.jsx`.
