...
### Task 4: Update equation-sim-map.ts (MANDATORY)

**This task is MANDATORY for every simulation.** Without it, the "Simulate This Equation" buttons in the article will not work.

**File:** `lib/equation-sim-map.ts`

1. Add the new slug to the `SimulationType` union type
2. Add equation string → simulation mapping entries for EVERY major equation in the article
3. The `type` field MUST match the `simType` prop on the LabContainer
4. The LabContainer MUST have `id={"lab-[slug]"}` — this is what `equation-block.tsx` scrolls to when "Simulate This Equation" is clicked

**CRITICAL:** If you skip this, the article will have dead "Simulate This Equation" buttons. This is a user-facing bug.
...
- [ ] If simulation exists: equation-sim-map.ts has at least ONE equation mapped for this simulation
- [ ] If simulation exists: LabContainer `id` is `lab-{slug}` so "Simulate This Equation" can scroll to it
- [ ] If simulation exists: `SimulationType` union in equation-sim-map.ts includes the new slug
