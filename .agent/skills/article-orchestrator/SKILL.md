...
3. **MANDATORY:** Check today's date using the `date` command in terminal. Use this for the `publishedAt` field later.
4. Present analysis and ask these questions conversationally:
...
2. Follow the build-master instructions to:
   - **If simulation EXISTS:**
     ...
     d. **MANDATORY:** Update `lib/equation-sim-map.ts` — add at least ONE equation mapping
     e. Verify LabContainer `id` is `lab-{slug}` so "Simulate This Equation" scrolls to it
     f. Verify `isPlaying` starts as `false` (no auto-play on load)
   - **If NO simulation:**
...
- **Build fails / Runtime bugs:**
  - USE the `@[/systematic-debugging]` workflow immediately.
